    ...
    <main className="mx-auto w-full max-w-3xl px-4 py-6 text-slate-800 sm:px-6">
      ...
      <header className="rounded-2xl bg-[#2b3648] px-6 py-5 text-white shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-sm font-semibold tracking-tight text-emerald-300">
              <span className="inline-block h-2 w-2 rounded-full bg-emerald-400" />
              {s.구역명} {s.주체}
            </div>
            <h1 className="mt-1 text-2xl font-bold sm:text-3xl">동의 현황 대시보드</h1>
            <p className="mt-1 text-xs text-slate-300">한가람 한양 · 삼성 · 두산 통합재건축</p>
          </div>
          <div className="text-right text-xs text-slate-300">
            <span className="rounded border border-slate-400 px-2 py-0.5 font-semibold tracking-widest">
              OFFICIAL
            </span>
      ...
      <section className="mt-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-end justify-between">
          <div>
            <div className="text-sm text-slate-500">전체 동의율</div>
          ...
          {d.단지들.map((c) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="w-16 shrink-0 text-right text-lg font-bold text-emerald-600">
                  {pct(c.동의율)}
                </span>
              </div>
            </Link>
      ...
      <footer className="mt-6 pb-4 text-center text-xs text-slate-400">
        {s.구역명} {s.주체} · 데이터는 구글시트 실집계 기준 · 5~10분 간격 자동 갱신
      </footer>
    </main>
  );
}
...
function Stat({
  ...
  return (
    <div className="bg-white px-4 py-3">
      ...
      <div className={`mt-0.5 text-2xl font-bold ${accent ? "text-emerald-600" : "text-slate-800"}`}>
        {value}
...
function DdayBadge({ d, 마감 }: { d: number; 마감: string }) {
  ...
  return (
    <div className="rounded-xl bg-slate-900 px-4 py-2 text-right text-white">
      ...
      <div className="mt-1 text-[11px] text-slate-300">접수마감 {마감}</div>
    </div>
  );
}
