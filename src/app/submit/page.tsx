  ...
  return (
    <main className="mx-auto w-full max-w-3xl px-4 py-6 text-[#2a2723] sm:px-6">
      ...
      <section className="mt-5 rounded-2xl border border-[#ece7dc] bg-white p-6 shadow-sm">
        ...
        <ol className="list-decimal space-y-1.5 pl-5 text-sm leading-relaxed text-[#4a453d]">
      ...
      {SUBMIT.map((info) => (
        <section
          key={info.단지}
          className="mt-5 overflow-hidden rounded-2xl border border-[#ece7dc] bg-white shadow-sm"
        >
          <div className="flex items-center justify-between border-b border-[#f0ebe0] px-6 py-4">
            <h2 className="serif text-lg font-bold">
              {info.단지} <span className="text-sm font-normal text-[#8b8578]">관리사무소</span>
            </h2>
            {info.전화 && (
              <a
                href={`tel:${info.전화.replace(/-/g, "")}`}
                className="rounded-lg bg-[#faf3e2] px-3 py-1.5 text-sm font-bold text-[#a4791f] ring-1 ring-[#a4791f]/30"
              >
                📞 {info.전화}
              </a>
            )}
          ...
          <div className="p-6">
            <p className="text-sm leading-relaxed text-[#4a453d]">{info.위치설명}</p>

            {info.mapImage ? (
              <div className="mt-4 overflow-hidden rounded-xl border border-[#ece7dc]">
                <Image
                  src={info.mapImage}
              ...
              <div className="mt-4 rounded-xl border border-dashed border-[#d8cfbc] bg-[#faf8f2] px-4 py-8 text-center text-sm text-[#8b8578]">
                🗺️ 위치 지도 준비 중입니다.
              </div>
            )}
            ...
            {info.mapLink && (
              <a
                ...
                className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-[#1c1c22] px-4 py-2 text-sm font-semibold text-[#e8c977]"
              >
                🧭 지도앱에서 단지 위치 열기
              </a>
            )}
      ...
      <p className="mt-4 rounded-xl bg-[#f3eee2] px-4 py-3 text-xs leading-relaxed text-[#7a7364]">
        ※ 관리사무소는 지도 앱에 주소가 등록되어 있지 않을 수 있어, 실제 위치를 이미지로 안내합니다.
        정확한 <b>접수 시간·준비물</b>과 삼성·두산 <b>위치 지도</b>는 확정되는 대로 업데이트됩니다.
      </p>

      <footer className="mt-6 pb-4 text-center text-xs text-[#8b8578]">
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-[#faf8f2] px-4 py-3">
      ...
      <div className="mt-0.5 text-sm font-semibold text-[#2a2723]">{value}</div>
    </div>
  );
}
